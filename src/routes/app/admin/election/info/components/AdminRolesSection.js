...
const AdminRolesSection = (props: Props) => {
  const {
    active, setActive, electionGroup, closeAction
  } = props;
  ...
  const adminPersons = adminRoles.filter(
    r => r.principal.principalType === 'person-principal'
  ).map(r => ({
  ...
  const adminGroups = adminRoles.filter(
    r => r.principal.principalType === 'group-principal'
  ).map(r => ({
  ...
  return (
    <PageSection header={<Trans>election.adminRoles</Trans>}
      desc={active ?
        <Trans>election.adminRolesDesc</Trans> : null}
      active={active}
      setActive={setActive}>
      {active ?
        <ApolloConsumer>
                  ...
                  <Mutation
                    ...
                    {(addAdmin, { data: addData }) => {
                      const addAction = (adminId, type) => {
                        addAdmin({
                          variables: {
                            adminId, type, elGrpId: electionGroup.id
                          }
                        });
                      };
        ...
        <AdminRolesValues roles={electionGroup.roles} />
      }
    </PageSection>
  )
}
